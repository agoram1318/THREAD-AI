...
const buildPrompt = (preset: string, items: GenerateThreadRequestItem[]) => {
  ...
  return [
    ...
    '5) 문단은 짧게 나누고, 번호형 구조(1️⃣, 2️⃣, 3️⃣)를 유지합니다.',
    '6) 이모지는 글당 3~5개만 사용하고, 반드시 아래 목록에서만 선택합니다: 📌 📈 📉 🏦 ⚠️ 💬',
    '7) 사실 기반으로 작성하고 단정/선정/과도한 확신 표현을 피합니다.',
    '8) 투자 관련 내용은 매수/매도 추천처럼 보이지 않게 작성합니다.',
    '9) 본문에 긴 URL을 나열하지 마세요.',
    '10) 마지막 줄은 반드시 "참고: 출처명1, 출처명2" 형식으로 간단히 표시합니다. (예: 참고: MarketWatch, Federal Reserve)',
    '11) 마지막에 "※ 투자 추천이 아닌 시장 흐름 정리입니다." 문구를 포함합니다.',
    '',
    '[프리셋 특화 규칙]',
    ...selectedPresetRules.map((rule, idx) => `${idx + 1}) ${rule}`),
    '',
    '[출력 형식 예시 지침]',
    '첫 문장은 핵심을 짚는 짧은 후킹으로 시작합니다. (예: 📌 오늘 미국 증시에서 볼 포인트는 3개입니다.)',
    '이후 번호형 항목 3개 내외로 핵심을 설명합니다.',
    '중간/마무리에 ⚠️ 정리 문단을 넣고, 마지막에는 💬 질문형 문장으로 마무리할 수 있습니다.',
